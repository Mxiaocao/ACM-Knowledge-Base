...
# Codeforces 最近 100 场：Div.2 B / Div.3 C 模型分析
...
**常见变换：** 恒等式消元、rad(n)、三进制、展开反射、距离度量。
...
**常见变换：** 万能 gadget、前缀 XOR、补排列、符号块、局部环。
...
**核心识别：** 题面是动态过程，但先找记录点、零段、循环/链等静态结构再模拟。

**常见变换：** 记录最大值、动态 successor、队列窗口、记忆化有向图。
...
**核心识别：** 把反复操作看成可达关系，寻找余数类、奇偶、连通分量、正规形等不会被改变的量。

**常见变换：** 余数类、奇偶类、连通分量、压缩块、规范余数。
...
**核心识别：** 通过消除、失配段、括号前缀或少量自动机状态压缩字符串过程。

**常见变换：** 栈消除、两模板失配、内部平衡点、4 状态自动机。
...
**核心识别：** 逐位独立、XOR 前缀、GF(2) 约束或最高位结构。

**常见变换：** 最高位判定、前缀 XOR 唯一碰撞、逐位可行模式。
...
**核心识别：** 答案由阈值、平均、最大缺口或单调 check 决定，常配合二分/水位思想。

**常见变换：** 前缀平均、水位、矩形包含、最长零段、阈值半轴。
...
A 级 87 题；B 级 13 题。B 级题已核对题面并完成主模型推导，但由于比赛较新或公开题解材料较少，正式做题前建议再读官方 editorial 核边界。Excel 中可直接按"核验等级=B"筛选。
